...
def analyze_industry_trends(g, industry_name):
    ...
    query = """SELECT ?company_name ?date ?amount ?phase
    WHERE {
    ...
    # Proper parametrization using Literal to avoid injection issues
    ...
    if len(years) >= 3:
        ...
        # Check for acceleration/deceleration with proper error handling
        if len(recent_funding) >= 2:
            # Safer division with checks for zero
            growth_rate_1 = (recent_funding[1] - recent_funding[0]) / recent_funding[0] if recent_funding[0] > 0 else 0
            growth_rate_2 = (recent_funding[2] - recent_funding[1]) / recent_funding[1] if recent_funding[1] > 0 else 0
            acceleration = "accelerating" if growth_rate_2 > growth_rate_1 else "decelerating"
            print(f"Growth is {acceleration}")

def main():
    """Main function to initialize graph and run analysis"""
